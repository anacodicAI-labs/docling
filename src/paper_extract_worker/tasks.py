...
def _process_single_file(db: Session, job: Job, job_file: JobFile) -> None:
    ...
    try:
        result = extract_paper_pdf(
            ...
            ocr_engine=options.get("ocr_engine", "auto"),
            ocr_full_page=bool(options.get("ocr_full_page", False)),
            ocr_lang=options.get("ocr_lang") or None,
            formulas=bool(options.get("formulas", False)),
            code_enrichment=bool(options.get("code_enrichment", False)),
            picture_classification=bool(options.get("picture_classification", False)),
            picture_description=bool(options.get("picture_description", False)),
            ...
            chunk=bool(options.get("chunk", False)),
            chunk_tokenizer=options.get("chunk_tokenizer", "sentence-transformers/all-MiniLM-L6-v2"),
            chunk_max_tokens=options.get("chunk_max_tokens"),
            chunk_overlap=int(options.get("chunk_overlap", 0)),
        )
        job_file.status = FileStatus.COMPLETED
